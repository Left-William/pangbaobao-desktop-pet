...
状态：**可运行的关键姿势预览，未通过正式动作验收**。这里记录首批伸展和跳跃；随后接入的俯卧撑见[横向舞台检查](pushup-wide-stage-2026-09-25.md)。本记录对应工作区素材；公开 GitHub Release 仍为 0.3。批准的人物母版 `pbb-photo-05-approved` 没有改动，用户之前确认的外貌方向为“基本像，可继续”。原始参考照片、未采用的生成候选与制作缓存留在 Git 忽略目录。
